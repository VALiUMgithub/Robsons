    ...
    const [dateRange,setDateRange] = useState([
      ...
      const generateReport = () => {
        axios({
          ...
          const fileName = `RobsonsClassificaton_Report_${moment(dateRange[0].startDate).format('YYYY-MM-DD')}_${moment(dateRange[0].endDate).format('YYYY-MM-DD')}_${moment(new Date()).format('x')}${randomNumber}.xlsx`;
          link.href = url;
    ...
    <><div className="flex flex-col items-center justify-center h-screen">
            ...
            <h1 className="text-3xl font-bold mb-4">Welcome {auth.user}</h1>
               ...
               <div className="flex flex-col justify-center mt-5">
                    <button className="bg-green-500 hover:bg-green-700 text-white font-bold py-2 px-4 rounded m-2" onClick={()=>generateReport()}>GENERATE REPORT</button>
               </div>
             </div>
            }       
