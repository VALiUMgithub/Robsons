...
function ObsIndexForm() {
  ...
  useEffect(() => {
    axios.get('http://localhost:3050/api/form-data')
      .then(response => {
  ...
  const goToPreviousForm = () => {
    if (formData[formIndex]?.showPrevious && previousFormIndexStack.length > 0) {
  ...
  const goToNextForm = () => {
    ...
    const condition = formData[formIndex]?.conditions.find(c => c.option === selectedOption);
    if (condition) {
      const targetFormIndex = formData.findIndex(f => f.title === condition.target);
  ...
  const submitForms = () => {
    ...
    axios.post('http://localhost:3050/submit-form', selectedOptions)
    .then(response => {
  ...
  const updateStatus = () => {
    ...
    axios.post('http://localhost:3050/api/update-status', {formId, indicationForInduction})
    .then(response => {
      console.log(response.data);
      let groupDetails = response.data;
      if(groupDetails){
      ...
      <ToastContainer />
        <div className=' bg-white shadow-lg rounded-lg '>
            <div className=" bg-green-400 rounded-t-lg pr-20 pl-2 py-1">
                <h2 className="text-2xl text-left font-bold">Robsons Classification</h2>
                {/* radio buttons */}
            </div>
                ...
                <div className="flex form-title mb-4 pr-20 pl-2">
                    <h3 className="text-lg font-bold">{formData[formIndex]?.displayText}</h3>
                </div>
                  ...
                  <div key={index}>
                    <label key={index} className="inline-flex items-center">
                      <input type="radio"
                        ...
                        onChange={() => updateThisOption(formData[formIndex]?.title, option.displayText)} 
                      />
                      <span className="ml-2">{option.displayText}</span>
                    </label>
                      
                  {option.type && option.type === 'textBox' && selectedOptions[formData[formIndex]?.title] === option.displayText && (
            ...
            {/* if date time picker is true then show the date time picker */}
                ...
                {formData[formIndex]?.dateAndTimePicker  && (
                      ...
                      onClose = {(value) => {
                          ...
                          updateThisOption(formData[formIndex]?.timeOfBirth, formattedTime);
                        }
                        }
                        />
                    </div>
                ...
                {formData[formIndex]?.showPrevious && (
                    <button onClick={goToPreviousForm} className=" text-white  hover:text-gray-800  rounded-bl-lg font-bold py-2 px-4  mr-auto">
                    Previous
                    </button>
                )}
                {formData[formIndex]?.showNext && (
                    <button onClick={goToNextForm} className=" text-white hover:text-gray-800  rounded-br-lg font-bold py-2 px-4  ml-auto">
                    Next
                    </button>
                )}
                {formData[formIndex]?.isSubmit && (
                    <button onClick={submitForms} className=" text-white hover:text-gray-800 bg-green-700  rounded-br-lg font-bold py-2 px-4  ml-auto">
                    Submit
                    </button>
                )}
                </div>
       ...
       {footerData.map(item => (
          <p key={item.key} className="text-md">{item.displayText}</p>
        ))}
       </div>
