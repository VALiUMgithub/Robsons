...
const app = express();
const totalGroupList = ['group 1','group 2','group 3','group 4','group 5','group 6','group 7','group 8','group 9','group 10'];
...
app.post('/submit-form', (req,res)=>{
  ...
  let highestMatchedGroup = { percentage: 0 };
    _.forEach(groupLogics, (logics) => {
        ...
        const percentage = calculatePercentageMatch(data, thisGroupLogic.conditions);
        if (percentage > highestMatchedGroup.percentage) {
          highestMatchedGroup = { groupId: thisGroupLogic.id, percentage: percentage };
        }
      })
    })
    ...
    ) VALUES ( "${data.obs_index}", "${data.weeks}", "${data.pog}", "${data.single_twins}", "${actualPreviousCesarean}", "${data.present}", ${data.Labour ? `"${data.Labour}"` : "null"}, ${data.delivery ? `"${data.delivery}"` : "null"}, "${data.indeovp}", "${data.IndeCS}", "${data.Stage}", "${data.BabyDetails}", ${data.date_of_birth ? `"${data.date_of_birth}"` : "null"}, ${data.time_of_birth ? `"${data.time_of_birth}"` : "null"}, "${data.weight}", "${data.apgar}", "${data.outcome}", "${data.indication}", "${data.final_outcome}", ${data.ripening ? `"${data.ripening}"` : "null"}, "${group}", "${data.created_by}", NOW()
    )
  `;
  con.query(sql, (err, result) => {
    ...
    con.query(groupQuery, (err, result) => {
      if (err) {
        ...
        res.status(400).send({ message: 'Error while inserting data in groups' });
        return;
      }
      
    });
    let responseData = { message: 'Data inserted successfully', group: group, formId: robsonsId };
...
app.get('/api/generate-report', (req, res) => {
  ...
  con.query(
    ...
    (error, robsonsDataList) => {
      ...
      _.forEach(robsonsDataList, (thisRobsonData) => {
        let tmpObject = {
            obs_index: !_.isEmpty(thisRobsonData.obs_index) ? thisRobsonData.obs_index : '',
            weeks: !_.isEmpty(thisRobsonData.weeks) ? thisRobsonData.weeks : '',
            pog: !_.isEmpty(thisRobsonData.pog) ? thisRobsonData.pog : '',
            single_twins: !_.isEmpty(thisRobsonData.single_twins) ? thisRobsonData.single_twins : '',
            previous_cesarean: !_.isEmpty(thisRobsonData.previous_cesarean) ? thisRobsonData.previous_cesarean : '',
            present: !_.isEmpty(thisRobsonData.present) ? thisRobsonData.present : '',
            Labour: !_.isEmpty(thisRobsonData.Labour) ? thisRobsonData.Labour : '',
            delivery: !_.isEmpty(thisRobsonData.delivery) ? thisRobsonData.delivery : '',
            indeovp: !_.isEmpty(thisRobsonData.indeovp) ? thisRobsonData.indeovp : '',
            IndeCS: !_.isEmpty(thisRobsonData.IndeCS) ? thisRobsonData.IndeCS : '',
            Stage: !_.isEmpty(thisRobsonData.Stage) ? thisRobsonData.Stage : '',
            BabyDetails: !_.isEmpty(thisRobsonData.BabyDetails) ? thisRobsonData.BabyDetails : '',
            date_of_birth: moment(thisRobsonData.date_of_birth).format('ddd D MMM YYYY'),
            time_of_birth: !_.isEmpty(thisRobsonData.time_of_birth) ? thisRobsonData.time_of_birth : '',
            weight: !_.isEmpty(thisRobsonData.weight) ? thisRobsonData.weight : '',
            apgar: !_.isEmpty(thisRobsonData.apgar) ? thisRobsonData.apgar : '',
            outcome: !_.isEmpty(thisRobsonData.outcome) ? thisRobsonData.outcome : '',
            indication: !_.isEmpty(thisRobsonData.indication) ? thisRobsonData.indication : '',
            final_outcome: !_.isEmpty(thisRobsonData.final_outcome) ? thisRobsonData.final_outcome : '',
            ripening: !_.isEmpty(thisRobsonData.ripening) ? thisRobsonData.ripening : '',
            group_name: !_.isEmpty(thisRobsonData.group_name) ? thisRobsonData.group_name : '',
            created_by: !_.isEmpty(thisRobsonData.created_by) ? thisRobsonData.created_by : '',
            created_on: moment(thisRobsonData.created_on).format('ddd D MMM YYYY'),
        }
        santizedRobsonsDataList.push(tmpObject);
      })

      let fieldNames = ['Obs Index', 'Weeks', 'POG', 'Single/Twins', 'Previous Cesarean', 'Present', 'Labour Type', 'Delivery', 'IndeOVP', 'IndeCS', 'Stage', 'Baby Details', 'Date of Birth', 'Time of Birth', 'Weight', 'APGAR', 'Outcome', 'Indication', 'Final Outcome', 'Ripening', 'Group', 'Created By', 'Created On'];
      let fields = ['obs_index', 'weeks', 'pog', 'single_twins', 'previous_cesarean', 'present', 'Labour', 'delivery', 'indeovp', 'IndeCS', 'Stage', 'BabyDetails', 'date_of_birth', 'time_of_birth', 'weight', 'apgar', 'outcome', 'indication', 'final_outcome', 'ripening', 'group_name', 'created_by', 'created_on'];
      let groupSpecification = {};
        _.forEach(fields, function (item, index) {
          groupSpecification[item] = { // <- the key should match the actual data key
  ...
  const calculateRelativeGroupSize = (groupsList, count_total) => {
    ...
    _.forEach(totalGroupList, (thisGroup) => {
      ...
      relativeGroupSize.push({ group_name: thisGroup, count:count, relativeGroupSize : percentage });
    })
  ...
  const calculateCSRateForEachGroup = async (groupsList, dateRangeOptions , res , relativeGroupSize) => {
    ...
    _.forEach(totalGroupList, async (thisGroup, index) => {
      ...
      con.query(robsonsQuery, async (error, result, fields) => {
        if (error) {
          console.error(error);
          res.status(500).send({ message: 'Internal Server Error calculateCSRateForEachGroup' });
          return;
        }
    ...
    // function to calculate CS rate for Each group
    const calculateRelativeCsRate = async (groupsList, dateRangeOptions , res , relativeGroupSize) => {
      try{
      let csRateForEachGroup = [];
      let currentCalculation = _.cloneDeep(relativeGroupSize);
      _.forEach(totalGroupList, async (thisGroup, index) => {
...
app.get('/api/generate-status-init', async (req, res) => {
  ...
  con.query(groupsQuery, async (error, result) => {
    if (error) {
      console.error(error);
      res.status(500).send({ message: 'Internal Server Error generate-status-init' });
      return;
    }
    ...
    let relativeGroupSize = await calculateRelativeGroupSize (groupsList, count_total);
    console.log("284",relativeGroupSize)
    const sanitizedStatus = relativeGroupSize.map(obj => _.omit(obj, 'count'));
